...
- **并行上传**：4 个并发上传通道，大幅提升多文件/目录上传速度
- **并行下载**：支持多选批量下载，4 个并发通道
- **实时下载进度**：分块传输实时显示每个文件的下载进度和百分比
- **大文件压缩下载**：超过 100MB 的文件自动探测远程压缩工具（zstd / pigz / gzip / bzip2 / xz），弹窗提示是否先压缩后下载
- **上传冲突处理**：支持覆盖/跳过/重命名选择
- 支持拖拽上传、右键删除
- 操作日志记录

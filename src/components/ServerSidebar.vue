<script setup lang="ts">
  ...
  import { Plus, Server as ServerIcon, Globe, MoreVertical, FileUp, FileDown, LogOut, Edit2, Trash2, Copy, Loader2 } from 'lucide-vue-next';
...
<template>
  <aside class="flex flex-col h-full relative" @click="activeMenuId = null" @contextmenu="handleContextMenu($event)">
    ...
    <div class="flex-1 overflow-y-auto p-2 space-y-1">
      <!-- Loading State -->
      <div v-if="serverStore.isInitializing" class="flex flex-col items-center justify-center py-10 space-y-3 opacity-50">
        <Loader2 class="animate-spin text-blue-500" :size="24" />
        <span class="text-[10px] uppercase tracking-widest font-bold">正在初始化数据库...</span>
      </div>

      <!-- Empty State -->
      <div v-else-if="serverStore.servers.length === 0" class="flex flex-col items-center justify-center py-12 px-4 text-center opacity-40">
        <div class="p-3 bg-slate-800 rounded-2xl mb-4">
          <ServerIcon :size="32" />
        </div>
        <p class="text-xs font-medium text-slate-300">暂无服务器</p>
        <p class="text-[10px] text-slate-500 mt-1">点击右上角“操作”添加您的第一个 SSH 会话</p>
      </div>
